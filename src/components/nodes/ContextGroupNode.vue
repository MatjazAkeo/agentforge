...
const memberCount = computed(
  () =>
    graph.edges.filter(
      (e) => e.target === props.id && (e.targetHandle ?? '') === 'contexts',
    ).length,
);
...
<template>
  ...
  >
    <div class="relative rounded-t-md flex items-center gap-2 px-3 py-1.5 border-b border-border-base">
      <span class="w-2 h-2 rounded-full bg-[#b388ff] flex-shrink-0" title="context group" />
      <div class="flex-1 min-w-0">
        <div class="text-text-base font-medium text-xs leading-tight">Context Group</div>
    ...
    <div class="relative h-6 flex items-center justify-between px-3 text-[11px]">
      <span class="text-text-dim font-mono text-[10px]">contexts</span>
      <Handle id="contexts" type="target" :position="Position.Left" :style="{ background: colorForType('context') }" />
    ...
    <div class="rounded-b-md px-3 py-1.5 text-[10px] opacity-60 border-t border-border-base bg-node-inset text-center">
      <span v-if="memberCount > 0">{{ memberCount }} input{{ memberCount === 1 ? '' : 's' }}</span>
      <span v-else class="italic">— no contexts yet —</span>
    </div>
  </div>
</template>
