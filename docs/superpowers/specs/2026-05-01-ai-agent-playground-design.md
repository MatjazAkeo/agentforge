...
- **Streaming into chat bubble deferred** — spec §9.4 calls for streaming tokens into the in-progress assistant bubble. v1 ships the simpler "running… then full reply" UX. Adding streaming requires the chat panel to trace the chat-output's upstream node back to its streaming source and read `runStore.livePreviews[id]` reactively — a follow-up if the simpler UX bothers anyone.
