...
describe('chat attachments', () => {
  ...
  it('adds an attachment', () => {
    const c = useChatStore();
    c.addAttachment({ filename: 'a.txt', content: 'hello', sizeBytes: 5 });
    expect(c.attachments).toEqual([{ filename: 'a.txt', content: 'hello', sizeBytes: 5 }]);
  });

  it('removes an attachment by index', () => {
    const c = useChatStore();
    c.addAttachment({ filename: 'a.txt', content: 'A', sizeBytes: 1 });
    c.addAttachment({ filename: 'b.txt', content: 'B', sizeBytes: 1 });
  ...
  it('composeUserMessage wraps each attachment in XML and appends after user text', () => {
    const c = useChatStore();
    c.addAttachment({ filename: 'a.txt', content: 'AA', sizeBytes: 2 });
    c.addAttachment({ filename: 'b.json', content: 'BB', sizeBytes: 2 });
